...
export const FastDobInput: React.FC<FastDobInputProps> = ({ value, onChange }) => {
  ...
  // Handle prop changes from parent (e.g. form reset or value set from outside)
  useEffect(() => {
    if (!value) {
      setDay('');
      setMonth('');
  ...
  // Helper to validate and notify parent when user edits day/month/year
  const notifyParent = (dStr: string, mStr: string, yStr: string) => {
      ...
      {ageString && ageNum !== null && (
        <div className="flex items-center justify-between bg-gradient-to-r from-orange-700 via-amber-600 to-orange-600 text-white px-2.5 py-1 rounded-xl shadow-xs border border-orange-300/40 animate-fade-in mt-1">
          <span className="text-[11px] font-bold flex items-center gap-1.5">
            ...
            <span>Age:</span>
            <strong className="font-mono text-xs font-black text-amber-200">{ageString}</strong>
          </span>
          {ageNum < 15 ? (
